...
int l2r_lr_fun::get_nr_pair(void)
{
	return -1;
}
...
void l2r_lr_fun::eval(double *w, double *result) 
{
	result = NULL;
	return;
}
...
int l2r_l2_svc_fun::get_nr_pair(void)
{
	return -1;
}
...
void l2r_l2_svc_fun::eval(double *w, double *result)
{
	result = NULL;
	return;
}
...
static void train_one(const problem *prob, const problem *prob_t, const parameter *param, double *w, double Cp, double Cn)
{
	double eps=param->eps;
	int pos = 0;
	int neg = 0;
	...
	switch(param->solver_type)
	{
		...
		{
			fun_obj=new l2r_l2_ranksvm_fun(prob, prob_t, param->C, param->split_type, param->acc_Hv);
			TRON tron_obj(fun_obj, param->eps, 10000, param->split_type);
			tron_obj.set_print_string(liblinear_print_string);
			tron_obj.tron(w);
			delete fun_obj;
			break;
...
// Interface functions
//
model* train(const problem *prob, const problem *prob_t, const parameter *param)
{
	int i,j;
	int l = prob->l;
	int n = prob->n;
	...
	if(check_regression_model(model_) || param->solver_type == L2R_L2LOSS_RANKSVM)
	{
		...
		model_->nr_class = 2;
		model_->label = NULL;
		train_one(prob, prob_t, param, &model_->w[0], 0, 0);
	}
	else
	{
		int nr_class;
		...
		else
		{
			if(nr_class == 2)
			{
				...
				train_one(&sub_prob, prob_t, param, &model_->w[0], weighted_C[0], weighted_C[1]);
			}
			else
			{
				...
				for(i=0;i<nr_class;i++)
				{
					...
					train_one(&sub_prob, prob_t, param, w, weighted_C[i], param->C);
...
double predict_values(const struct model *model_, const struct feature_node *x, double *dec_values)
{
	...
	if(model_->param.solver_type == L2R_L2LOSS_RANKSVM && model_->param.split_type == FW)
		mpi_allreduce(dec_values, nr_w, MPI_DOUBLE, MPI_SUM);
...
int save_model(const char *model_file_name, const struct model *model_)
{
	...
	{
		if(model_->param.split_type == QW)
			fprintf(fp, "split_type QW\n");
		else
			fprintf(fp, "split_type FW\n");
	}
...
struct model *load_model(const char *model_file_name)
{
	...
	while(1)
	{
		...
		else if(strcmp(cmd,"split_type")==0)
		{
			fscanf(fp,"%80s",cmd);
			if(strcmp(cmd, "QW")==0)
				model_->param.split_type=QW;
			else
				model_->param.split_type=FW;
		}
...
const char *check_parameter(const problem *prob, const parameter *param)
{
	...
	if(param->solver_type != L2R_L2LOSS_RANKSVM
		&& mpi_get_size() > 1)
		return "distributed computation is only supported for l2-loss ranksvm";
